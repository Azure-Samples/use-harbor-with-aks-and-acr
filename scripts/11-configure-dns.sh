...
echo "Retrieving the external IP address of the NGINX ingress controller..."
...
if [ -n $publicIpAddress ]; then
  echo "[$publicIpAddress] external IP address of the NGINX ingress controller successfully retrieved"
else
  echo "Failed to retrieve the external IP address of the NGINX ingress controller"
  exit
fi

# Check if an A record for the subdomain exists in the DNS Zone
echo "Retrieving the A record for the [$sampleSubdomain] subdomain from the [$dnsZoneName] DNS zone..."
ipv4Address=$(az network dns record-set a list \
  --zone-name $dnsZoneName \
  --resource-group $dnsZoneResourceGroupName \
  --query "[?name=='$sampleSubdomain'].ARecords[].ipv4Address" \
  --output tsv \
  --only-show-errors)

if [[ -n $ipv4Address ]]; then
  echo "An A record already exists in [$dnsZoneName] DNS zone for the [$sampleSubdomain] subdomain with [$ipv4Address] IP address"
  ...
  # Retrieving the name of the record set relative to the zone
  ...
  recordSetName=$(az network dns record-set a list \
    --zone-name $dnsZoneName \
    --resource-group $dnsZoneResourceGroupName \
    --query "[?name=='$sampleSubdomain'].name" \
    --output tsv \
...
# Create the A record
echo "Creating an A record in [$dnsZoneName] DNS zone for the [$sampleSubdomain] subdomain with [$publicIpAddress] IP address..."
az network dns record-set a add-record \
  --zone-name $dnsZoneName \
  --resource-group $dnsZoneResourceGroupName \
  --record-set-name $sampleSubdomain \
  --ipv4-address $publicIpAddress \
  --only-show-errors 1>/dev/null

if [[ $? == 0 ]]; then
  echo "A record for the [$sampleSubdomain] subdomain with [$publicIpAddress] IP address successfully created in [$dnsZoneName] DNS zone"
else
  echo "Failed to create an A record for the $sampleSubdomain subdomain with [$publicIpAddress] IP address in [$dnsZoneName] DNS zone"
fi
